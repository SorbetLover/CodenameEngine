///////////////// discarded class, attemping loading chars from another mod folder
...
class CharacterAlt extends FunkinSprite implements IBeatReceiver implements IOffsetCompatible implements IPrePostDraw {
	...
	public var gameOverCharacter:String = CharacterAlt.FALLBACK_DEAD_CHARACTER;
	...
	public function new(x:Float, y:Float, ?character:String, isPlayer:Bool = false, switchAnims:Bool = true, disableScripts:Bool = false)
	{
		...
		if(!disableScripts){
			// caminho de script no alloyshit (com extensão .hx)
			var scriptPath:String = 'mods/alloyshit/data/characters/$curCharacter.hx';
	
			// Se quiser suportar outros locais: checar Assets.exists em ordem
			if (Assets.exists(scriptPath)) {
				script = Script.create(scriptPath);
			} else {
				// fallback para o comportamento padrão (se necessário)
				var defaultScriptPath = Path.withoutExtension(Paths.xml('characters/$curCharacter'));
				script = Script.create(Paths.script(defaultScriptPath, null, true));
			}
	
			// Se ainda for null, deixa DummyScript para evitar crash mais adiante
		...
		// garantir que scripts não é null antes de chamar
		if (scripts != null) scripts.call("create");

		buildCharacter(xml);

		// chamar postCreate só se scripts existem
		if (scripts != null) scripts.call("postCreate");
	...
	public function dance() {
		...
		if (scripts != null) scripts.call("onDance", [event]);
	...
	public override function measureHit(curMeasure:Int)
		if (scripts != null) scripts.call("measureHit", [curMeasure]);
	...
	public override function draw() {
		...
		if (scripts != null) script.call("draw", [e]);

		preDraw();
		super.draw();
		postDraw();

		if (scripts != null) script.call("postDraw", [e]);
	}
	...
	public function playSingAnim(direction:Int, suffix:String = "", Context:PlayAnimContext = SING, ?Force:Null<Bool> = null, Reversed:Bool = false, Frame:Int = 0)
	{
		...
		if (scripts != null) script.call("onPlaySingAnim", [event]);
		if (event.cancelled) return;
	...
	public function playSingAnimUnsafe(direction:Int, suffix:String = "", Context:PlayAnimContext = SING, Force:Bool = true, Reversed:Bool = false, Frame:Int = 0) {
		var event = EventManager.get(DirectionAnimEvent).recycle(getSingAnim(direction, suffix), direction, suffix, Context, Reversed, Frame, Force);
		if (scripts != null) script.call("playSingAnimUnsafe", [event]);
		if (event.cancelled) return;
	...
	public function applyXML(xml:Access) { // just for now till i remake the dumb editor
		gameOverCharacter = CharacterAlt.FALLBACK_DEAD_CHARACTER;
	...
	public inline function buildCharacter(xml:Access) {
		...
		loadSprite('mods/alloyshit/images/characters/$sprite');
		for(node in xml.elements) {
			switch(node.name) {
				case "anim":
	...
	public inline function buildXML(?animsOrder:Array<String>):Xml {
		...
		if (gameOverCharacter != CharacterAlt.FALLBACK_DEAD_CHARACTER) xml.set("gameOverChar", gameOverCharacter);
	...
	public static function getXMLFromCharName(character:OneOfTwo<String, CharacterAlt>):Access {
		var char:CharacterAlt = null;
		if (character is CharacterAlt) {
			char = cast character;
		...
		var xml:Access = null;
		while (true) {
			var xmlPath:String = 'mods/alloyshit/data/characters/$character';
			trace("cu getxml");
			if (!Assets.exists(xmlPath)) {
			character = FALLBACK_CHARACTER;

			var fallbackPath = 'mods/alloyshit/data/characters/$character';
			if (!Assets.exists(fallbackPath)) {
				// Não tem fallback → sai do loop antes de travar
				break;
			}
